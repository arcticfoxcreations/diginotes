...
const supabase = (window.supabase && SUPABASE_URL.includes('supabase.co') && !SUPABASE_URL.includes('YOUR_PROJECT_ID'))
...
const THEMES = {
  romantic: {
    ...
    petals: 10, rings: 2, petalShape: 'round', floaty: 'petal', defaultVariant: 'blush'
  },
  blossom: {
    label: 'Cherry Blossom', flowerName: 'sakura',
    petals: 5, rings: 1, petalShape: 'notch', floaty: 'petal', defaultVariant: 'blush'
  },
  dreamy: {
    label: 'Dreamy Wildflower', flowerName: 'wildflower',
    petals: 7, rings: 1, petalShape: 'wide', floaty: 'butterfly', defaultVariant: 'lavender'
  },
  minimal: {
    label: 'Minimal Bloom', flowerName: 'daisy',
    petals: 11, rings: 1, petalShape: 'thin', floaty: 'clover', defaultVariant: 'sage'
  }
};
...
// Builds a soft, layered, gradient-shaded flower — same shape system as the
// occasion, coloured with whichever palette the person picked.
let svgIdCounter = 0;
function makeFlowerSVG(themeId, variantId, size){
  ...
  size = size || 100;
  const gid = 'petalGrad' + (svgIdCounter++);

  const ringDefs = t.rings === 2
    ? [ { n: t.petals, scale: 1, offset: 0 },
        { n: Math.max(4, Math.floor(t.petals * 0.6)), scale: .6, offset: (360 / t.petals) / 2 } ]
    : [ { n: t.petals, scale: 1, offset: 0 } ];
  ...
  ringDefs.forEach(ring => {
    for (let i = 0; i < ring.n; i++){
      ...
      const notch = t.petalShape === 'notch'
        ? `M50,${50 - ry*ring.scale*0.9} C${50 - rx*ring.scale},${50 - ry*ring.scale*0.5} ${50 - rx*ring.scale},${50 - ry*ring.scale*0.1} 50,${50 - ry*ring.scale*0.28}
           C50,${50 - ry*ring.scale*0.1} ${50 + rx*ring.scale},${50 - ry*ring.scale*0.5} 50,${50 - ry*ring.scale*0.9} Z`
        : null;
      if (notch){
        petals += `<path d="${notch}" fill="url(#${gid})" stroke="${v.deep}" stroke-width="0.5" stroke-opacity="0.3" transform="rotate(${angle} 50 50)"/>`;
      } else {
        petals += `<ellipse cx="50" cy="${50 - ry*ring.scale*0.42}" rx="${rx*ring.scale}" ry="${ry*ring.scale}"
          fill="url(#${gid})" stroke="${v.deep}" stroke-width="0.5" stroke-opacity="0.3"
  ...
  return `<svg viewBox="0 0 100 100" width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <radialGradient id="${gid}" cx="50%" cy="30%" r="75%">
        ...
        <stop offset="55%" stop-color="${v.center}"/>
        <stop offset="100%" stop-color="${v.accent}"/>
      </radialGradient>
    </defs>
    <path d="M50 62 C46 74 46 84 50 92 C54 84 54 74 50 62 Z" fill="#7a9d6a" opacity="0.55"/>
    <g>${petals}</g>
    <circle cx="50" cy="50" r="9" fill="${v.deep}" opacity="0.9"/>
  </svg>`;
}
...
function buildOccasionGrid(){
  ...
  THEME_ORDER.forEach(id => {
    ...
    card.innerHTML = makeFlowerSVG(id, t.defaultVariant, 56) + `<p>${t.label}</p>`;
...
function buildLetterCardHTML(state){
  const flower = makeFlowerSVG(state.theme, state.colorVariant, 100);
...
function renderPreview(){
  ...
  if (seamBtn && draft.music && draft.music.type === 'youtube') seamBtn.addEventListener('click', () => togglePreviewYouTube());
}

// Full-song loop: plays from the marked start to the natural end of the
// track, then jumps back to the start and keeps going — until someone
// pauses it from the music note (♪) icon.
let previewYtPlayer = null;
function togglePreviewYouTube(){
  loadYouTubeApi().then(() => {
    if (previewYtPlayer){
      const s = previewYtPlayer.getPlayerState();
      if (s === YT.PlayerState.PLAYING) previewYtPlayer.pauseVideo(); else previewYtPlayer.playVideo();
      return;
    }
    ...
    previewYtPlayer = new YT.Player(slot, {
      ...
      events: {
        ...
        onStateChange: (e) => { if (e.data === YT.PlayerState.ENDED){ e.target.seekTo(draft.music.start || 0, true); e.target.playVideo(); } }
      }
    });
  });
}
...
document.getElementById('btn-get-link').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase.from('letters').insert([{ data: payload }]).select();
  btn.disabled = false;
...
function renderRecipientView(state){
  ...
  if (state.music && state.music.type === 'youtube') setupRecipientYouTube(state.music, mount);
}
...
function setupRecipientYouTube(music, mount){
  ...
  loadYouTubeApi().then(() => {
    ...
    recipientYtPlayer = new YT.Player(slot, {
      ...
      events: {
        onReady: (e) => {
          // Try to autoplay with sound right away.
          e.target.playVideo();
          setTimeout(() => {
            const state = e.target.getPlayerState();
            if (state !== YT.PlayerState.PLAYING && state !== YT.PlayerState.BUFFERING){
              // Browser blocked unmuted autoplay — fall back to muted
              // autoplay plus a one-tap "unmute" pill so at least the
              // song is already going the instant they land on the page.
              e.target.mute();
              e.target.playVideo();
  ...
  if (seamBtn){
    seamBtn.addEventListener('click', () => {
      if (!recipientYtPlayer) return;
      const s = recipientYtPlayer.getPlayerState();
      if (s === YT.PlayerState.PLAYING) recipientYtPlayer.pauseVideo(); else recipientYtPlayer.playVideo();
    });
  }
}
