...
const THEMES = {
  ...
  dreamy: {
    label: 'Dreamy Wildflower', flowerName: 'clematis',
    petals: 6, rings: 1, petalShape: 'pointed', floaty: 'butterfly', defaultVariant: 'lavender'
  },
  minimal: {
...
function petalDims(shape){
  switch(shape){
    ...
    case 'pointed': return { rx: 15, ry: 27 };
...
function makeFlowerSVG(themeId, variantId, size){
  ...
  ringDefs.forEach(ring => {
    for (let i = 0; i < ring.n; i++){
      // small per-petal jitter so the bloom reads as hand-grown, not a
      // perfectly mechanical radial pattern
      const jitter = Math.sin(i * 12.9898) * 4;
      const angle = (360 / ring.n) * i + ring.offset + jitter;
      const s = ring.scale * (0.94 + 0.12 * Math.abs(Math.cos(i * 3.7)));
      const cy = 50 - ry * s * 0.42;

      if (t.petalShape === 'pointed'){
        // long, pointed clematis-style petal with a soft centre vein
        const top = 50 - ry * s, base = 50 - ry * s * 0.05;
        const w = rx * s;
        petals += `<path d="M50,${top} C${50 - w},${top + ry*s*0.55} ${50 - w*0.85},${base - ry*s*0.1} 50,${base}
          C${50 + w*0.85},${base - ry*s*0.1} ${50 + w},${top + ry*s*0.55} 50,${top} Z"
          fill="url(#${gid})" stroke="${v.deep}" stroke-width="0.5" stroke-opacity="0.35" transform="rotate(${angle} 50 50)"/>
          <path d="M50,${top + 4} L50,${base - 3}" stroke="#ffffff" stroke-width="0.6" stroke-opacity="0.4" transform="rotate(${angle} 50 50)"/>`;
      } else if (t.petalShape === 'notch'){
        petals += `<path d="M50,${50 - ry*s*0.9} C${50 - rx*s},${50 - ry*s*0.5} ${50 - rx*s},${50 - ry*s*0.1} 50,${50 - ry*s*0.28}
          C50,${50 - ry*s*0.1} ${50 + rx*s},${50 - ry*s*0.5} 50,${50 - ry*s*0.9} Z"
          fill="url(#${gid})" stroke="${v.deep}" stroke-width="0.5" stroke-opacity="0.3" transform="rotate(${angle} 50 50)"/>`;
      } else {
        petals += `<ellipse cx="50" cy="${cy}" rx="${rx*s}" ry="${ry*s}"
  ...
  // fine radiating stamens for the pointed (clematis) flower, like the reference
  let stamens = '';
  if (t.petalShape === 'pointed'){
    for (let i = 0; i < 14; i++){
      const a = (360/14) * i;
      stamens += `<line x1="50" y1="50" x2="50" y2="41" stroke="${v.deep}" stroke-width="0.5" stroke-opacity="0.6" transform="rotate(${a} 50 50)"/>`;
    }
  }
  ...
  return `<svg viewBox="0 0 100 100" width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
    ...
    <g>${petals}</g>
    ${stamens ? `<g>${stamens}</g><circle cx="50" cy="50" r="4.5" fill="${v.deep}" opacity="0.85"/>` : `<circle cx="50" cy="50" r="9" fill="${v.deep}" opacity="0.9"/>`}
  </svg>`;
}
